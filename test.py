...
import hw_vocoder.model as module_model

from hw_vocoder.trainer import Trainer
from hw_vocoder.utils import ROOT_PATH
from hw_vocoder.utils.object_loading import get_dataloaders
from hw_vocoder.utils import MetricTracker
...
def syn(model, input_dir, output_dir, device):
    model.eval()
    mel = MelSpectrogram()
    input_dir = Path(input_dir)

    for i in range(len(os.listdir(input_dir))):
        file_path = input_dir / f'audio_{i + 1}.wav'
        audio_tensor, sr = torchaudio.load(str(file_path))
        audio_tensor = audio_tensor[0:1, :]
        mel_spec = mel(audio_tensor)
...
def main(config, input_dir, out_dir):
    ...
    syn(model, input_dir, out_dir, device)
